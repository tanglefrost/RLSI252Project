...
def run_bandit(instance_means, m_list, T, algorithm, alpha=0.2):
    ...
    if algorithm == 'OTO':
        ...
        oto = SoftSwitchOTO(instance_means, m_list, T, alpha, temperature=2.0)
    for t in range(T):
        if algorithm == 'UCB':
...
def run_bandit_UH(instance_means, m_list, T, algorithm, alpha=0.6):
    '''
    instance_means: array, arm true means
    m_list: list, offline samples per arm
    T: horizon
    ...
    if algorithm == 'OTO':
        ...
        oto = SoftSwitchOTO(instance_means, m_list, 2, alpha, temperature=2.0)
    for t in range(T):
        if algorithm == 'UCB':
            mu_hat = np.divide(s, n, out=np.zeros_like(n, dtype=float), where=n>0)
            cb = conf_bound(n, K, delta)
